...
要比较同一段流水线的不同切法时，用 `bench` 生产变体×样例×种子的证据网格；它不替你
裁决或接线。每个变体都必须写出可检验的假设，并且恰有一个现状对照：

```python
from pathlib import Path

from kigumi import Judgment, LLMCaller, Variant, bench

transport = ...
cache_dir = Path("artifacts/_llm")
examples = [{"outline": "...", "reference": "..."}, ...]

variants = [
    Variant(
        "current",
        "整段一次生成是当前可用的对照",
        lambda example, caller: write_outline(example, caller),
        incumbent=True,
    ),
    Variant(
        "turn-split",
        "在叙事转折处切段可减少前后信息遗失",
        lambda example, caller: write_by_turns(example, caller),
    ),
    Variant(
        "beat-split",
        "按节拍切段可让局部约束更容易满足",
        lambda example, caller: write_by_beats(example, caller),
    ),
]
...
report = bench(
    variants,
    examples,
    metric=lambda example, output: Judgment(score_outline(example, output), "见人工评语"),
    caller_factory=lambda seed: LLMCaller(transport, cache_dir, seed=seed),
    report_path=Path("benches/2026-07-13-outline.json"),
)
```

三条纪律：

1. 报告只产证据不产裁决；谁赢、是否接线由人和上层流程决定。
2. 输家带着数字归档，不把未被采纳的结构实验抹掉。
3. 固定种子让加变体增量复用 L1 缓存；变体自身仍各自持有 caller，成本可按变体归集。
