...
3. `external` 摘要包含 `agent_executor_schema=2`、adapter/Pi expected version、bridge digest、
   capsule digest、provider/model/thinking、工具和 limits；`CACHE_SCHEMA` 保持 3。
...
11. env resolver 的值在写入 error、trajectory、RPC、stderr、Hook evidence 或 completion 前脱敏。
12. 成功与失败均保留 RPC、bounded stderr、normalized trajectory、Hook/policy evidence、usage、
    cost/stop reason、workspace manifest、attachments 与 exact publish；普通 materializer 不解释
    evidence attachment，GC 将其纳入 reachability。
13. v1 没有自动进化、winner/promotion、Agent factory、动态多 Agent 拓扑或 Agent map/scan；
...
必须递增 `agent_executor_schema` 并记入 CHANGELOG；只有改变普通键或 canonical artifact 语义才
递增 `CACHE_SCHEMA`。
