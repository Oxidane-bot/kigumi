...
| [执行准入契约](admission.md) | Draft (Unreleased) | 预算与节点资源怎样在副作用前准入，并如何共享并发平面？ |
| [Durable retry 与 run resume 契约](retry-resume.md) | Active (0.13.0) | retry、resume 与 ambiguous side effect 在什么边界上裁决？ |
| [Agent node 契约](agent-node.md) | Active (0.11.0) | 外部 Agent 与 Agent scan 怎样成为可缓存、可审计的 DAG 执行器？ |
...
| [守卫环与豁免契约](guards.md) | Active (0.13.0) | 四个守卫入口怎样拒绝裸循环调用和未声明文件读取？ |
