...
| 契约 | 状态 | 回答的问题 |
| --- | --- | --- |
| [缓存键契约](cache-key.md) | Active (0.8.0) | 哪些输入进入 L1/L3 内容键，什么变化必须换键或换族？ |
...
| [分层 Prompt 解析契约](prompt-resolution.md) | Active (0.7.0) | Prompt 声明怎样确定解析、入键并留下不含内容的 lineage？ |
...
| [检查点审批契约](checkpoint.md) | Active (0.7.0) | 人工批准怎样绑定精确 payload，并在同一 run 中恢复？ |
| [Failure 契约](failure.md) | Active (0.7.0) | CALL 与 Agent 怎样共享可序列化、provider-neutral 的失败事实？ |
| [Durable retry 与 run resume 契约](retry-resume.md) | Active (0.7.0) | retry、resume 与 ambiguous side effect 在什么边界上裁决？ |
| [Agent node 契约](agent-node.md) | Active (0.8.0) | 外部 Agent 与 Agent scan 怎样成为可缓存、可审计的 DAG 执行器？ |
...
| [WorkflowProfile 画像契约](workflow-profile.md) | Active (0.7.0) | 哪一份 IR 同时供应静态声明、运行检查、trace 与图渲染？ |
