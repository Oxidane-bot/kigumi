...
每份契约的 H1 下方必须直接出现一个且仅一个受认可的状态。已发布的规范契约使用
`Status: Active (X.Y.Z)`，其中版本表示这份契约文本最后一次实质修订所在的 release，
不是最低兼容版本；尚未发布的新契约使用 `Status: Draft (Unreleased)`，在 release 准备时
必须提升为实际的 release version。修改任一缓存键成分就是缓存换族，必须在同一次提交中更新
[`CHANGELOG.md`](../../CHANGELOG.md)。

| 契约 | 状态 | 回答的问题 |
| --- | --- | --- |
| [缓存键契约](cache-key.md) | Active (0.14.0) | 哪些输入进入 L1/L3 内容键，什么变化必须换键或换族？ |
| [确定性字节契约](determinism.md) | Active (0.14.0) | cold、warm 与重放路径怎样保持相同的 canonical 字节结果？ |
...
| [执行准入契约](admission.md) | Active (0.14.0) | 预算与节点资源怎样在副作用前准入，并如何共享并发平面？ |
| [Durable retry 与 run resume 契约](retry-resume.md) | Active (0.14.0) | retry、resume 与 ambiguous side effect 在什么边界上裁决？ |
| [Agent node 契约](agent-node.md) | Active (0.14.0) | 外部 Agent 与 Agent scan 怎样成为可缓存、可审计的 DAG 执行器？ |
...
| [守卫环与豁免契约](guards.md) | Active (0.14.0) | 四个守卫入口怎样拒绝裸循环调用和未声明文件读取？ |
