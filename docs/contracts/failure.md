...
4. `AgentRuntimeFailureCode` 只描述 spawn、version、process、protocol、policy、capacity；已知
   的 envelope、bridge policy、submit contract、config policy 事实可额外写入受限的
   `AgentRuntimeFailureSubCode`，但必须映射到兼容的宽码。provider 失败必须嵌入同一个
   `ProviderFailure`，两者不能同时存在。
...
6. CALL failed metadata、Agent failure JSON、attempt receipt 和 run manifest failure
   ...
   未知 Agent 异常不生成 sub-code；canonical failure 不保存原始异常文本或凭据。
