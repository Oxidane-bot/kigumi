...
def test_release_candidate_identity_is_explicit_and_unreleased_precedes_release() -> None:
    changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert __version__ == EXPECTED_RELEASE_VERSION
    unreleased = re.search(r"^## \[Unreleased\]$", changelog, re.MULTILINE)
    release = re.search(
        rf"^## \[{re.escape(EXPECTED_RELEASE_VERSION)}\] - {re.escape(EXPECTED_RELEASE_DATE)}$",
        changelog,
        re.MULTILINE,
    )
    assert unreleased is not None
    assert release is not None
    between = changelog[unreleased.end() : release.start()]
    assert between.strip(), "the Unreleased section must document pending user-facing changes"
    assert not re.search(r"^## ", between, re.MULTILINE), (
        "the release candidate must be the first dated section after Unreleased"
    )
