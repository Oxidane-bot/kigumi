...
├── _runstate.py    #     schema-2 run manifest、attempt receipt 与 candidate
├── _version.py     #     包版本单一来源
...
0.7 在这七个底层字节原语之上增加声明式组合层：`PromptSpec` 用一个 base、固定
`PromptLayer`、有限 `PromptAxis` 和 `PromptMaterial` 组成单个文本 Prompt。selector 只来自
已声明的 Input/Param/Item/Carry，严格 tuple path，无 fallback 或类型转换；material 仍只能
经 `inject()`。run 开始时一次读取完整 Prompt catalog snapshot，节点在 L3 lookup 前解析全部
