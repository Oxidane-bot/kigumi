...
├── _runstate.py    #     schema-1 run manifest、attempt receipt 与 candidate
├── _version.py     #     包版本单一来源
...
├── retry.py        #     显式 retry policy、确定性 schedule 与 terminal error
...
0.6 把 Agent canonical artifact 与 execution evidence 分开：artifact 只描述可消费结果，
RPC/stderr/trajectory/usage/workspace/slot 全部归 hash-bound origin provenance。
`EvidencePolicy` 只控制 retention，不是加密或访问控制。Agent miss 在 staging 前申请共享
slot；hit 不占容量。Pi hidden retry、thinking-off 违约与 observed model substitution 都
fail closed。
...
各层不得自带另一套发现逻辑。
三项 Agent capacity 可分别由同名 `KIGUMI_AGENT_*` 环境变量覆盖；跨项目共享机器级容量
必须显式指向同一 lock root。
...
- schema-1 `_run.json` 固定 graph/targets/force/source/libs/retry/evidence identity；旧 run
  只读不可恢复，声明漂移拒绝。provider/Pi side effect 前写 receipt；成功先写 candidate，
...
- node cache envelope schema 2 同时保存 canonical artifact 与 immutable origin；
  GC 从 retained sidecar、failure、attempt receipt 和 candidate 递归追踪 blob/attachment。

### enforce(守卫四环:注册环加三个外环)
1. 注册环:@node 注册时 AST 检查节点函数体，是精确且权威的边界；循环内裸调
   `.call(`/`.llm(` 或节点体直接 raw-io，且无对应理由豁免 → 拒绝注册。
   **边界诚实声明**:运行环只覆盖 dag 项目;命令式项目
   主责在测试环与提交环。
2. 检查环:`dag check` 对 source_dirs 做启发式扫描,不跑测试、不提交即可自查。
3. 测试环:pytest 插件自动收集守卫检查 + 全模板 dry-render 测试;
   **新增豁免注释即告警**(waiver 是 agent 的自助逃生通道,必须留痕上报)。
4. 提交环:`kigumi init --hooks` 把 pre-commit 写入 `.git/hooks/`
   (跑 `uv run kigumi guard --changed`;已有 hook 时拒绝覆盖);
   guard 输出含 waiver 清单。
...
### 格式化/质量
复用 ruff(lint+format),不自造工具;ruff/pytest 配置由项目自持,
`kigumi init` 不派发预设。
...
approve、diff、trace、call、gc。`dag.cli()` 是需要注册表的图入口:check、plan、graph、explain、
describe、resume、retry-resolve;两者不互相代替。
...
- 2026-07-24 发布 0.6.0：统一 typed failure、EvidencePolicy、默认单 slot 的全局 Agent
  ...
  `agent_schema=2`、cache envelope schema 2。0.5 cache 自然 miss，旧 run 不可 resume。

- 2026-07-14 文档对齐:包结构树补齐 bench.py、inspect.py、views.py,与 kigumi/ 实际
  23 个模块一致;CLI 清单与 `kigumi init` 描述修正为与 cli.py 实现一致
  (--hooks 显式安装、render/gc/trace/call 实签名、脚手架实际产物);
  守卫计数统一为注册环加三个外环。
